...
TypeDiagram("Hints API Example: Custom Anchors")
  .build(({ el, rel, hint }) => {
    ...
    // アンカーの位置を固定
    hint.getLayoutContext().createConstraints("anchor/centerX/value", (builder) => {
      builder.expr([1, centerX.variable]).eq([300, 1]).required()
    })
    
    hint.getLayoutContext().createConstraints("anchor/topY/value", (builder) => {
      builder.expr([1, topY.variable]).eq([100, 1]).required()
    })
    
    // Box1 を中央アンカーに左揃え、上アンカーに配置
    hint.getLayoutContext().createConstraints("box1/align", (builder) => {
    ...
    hint.getLayoutContext().createConstraints("box2/x", (builder) => {
      // Box2 も中央アンカーに左揃え
    ...
    hint.getLayoutContext().createConstraints("box3/x", (builder) => {
      // Box3 も中央アンカーに左揃え
