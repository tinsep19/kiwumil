...
- **`LayoutContext` の利用**: ファクトリは第2引数 `layout` を受け取り、`layout.variables.createBound(id)` で LayoutBound を生成し、`layout.constraints.withSymbol(...)` で symbol.ensureLayoutBounds(builder) を呼び出して制約を登録する
