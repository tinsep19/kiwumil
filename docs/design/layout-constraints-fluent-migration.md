...
- The new `ConstraintsBuilder` (per `docs/draft/new_constraint_builder.md`) is already wired into parts of `LayoutConstraints`, but several helpers, hints, and exported types still rely on `LayoutSolver.addConstraint`/`.expression` or legacy type wrappers.
- Keeping these old pathways prolongs the `LayoutSolver` surface, encourages duplicated expression logic, and pits downstream consumers (hints, tests, docs) against both the old and new APIs.
...
1. Ensure every constraint-producing helper in `src/layout` (constraints utilities, hint builders, layout helpers) issues constraints exclusively through `ConstraintsBuilder`.
...
- **Acceptance**: Each helper uses `createConstraintsBuilder()` rather than `LayoutSolver.expression`/`.addConstraint`, the `rawConstraints` recorded by `record()` still match their previous counts, and `LayoutConstraintType`/`strength` mappings are unchanged.
...
- **Acceptance**: No file outside `constraints_builder` calls `LayoutSolver.addConstraint`/`.expression`, guides/tests no longer rely on `LayoutContext.getSolver()` or private solver fields, and existing coverage still passes.
...
- **Steps**:
  ...
  3. Review `src/layout` exports for duplicates and ensure no stray `kiwi` imports remain.
...
- Removing `LayoutSolver` helpers must happen only after all consumers adopt the builder; this document is a guardrail before the final cut.
