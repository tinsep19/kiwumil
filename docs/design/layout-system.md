...
`LayoutContext` also provides helpers such as `expressionFromBounds` to build expressions from a symbol's bounds and convenience methods like `applyMinSize` / `anchorToOrigin` that wrap common constraint combinations for plugins and special symbols.
```
...
`LayoutVariables.createBound()` は solver を使って派生変数を生成し、派生変数の定義式を一度に追加します。
...
DiagramSymbol は構築時に `LayoutContext.constraints.withSymbol` を呼び出し、`LayoutConstraintBuilder` を通じて自身のレイアウト制約を登録します。たとえば `containerInbounds` の制約は以下のように追加されます。
...
`ensureLayoutBounds` は `buildContainerConstraints` と最小サイズ制約の両方を登録し、以下のような制約を `LayoutConstraintStrength.Strong`/`Weak` で追加します。
...
また `ensureLayoutBounds` は最初の呼び出し時に `bounds.x/y` を 0 に固定し（`Strong`）、`bounds.width/height` に `200/150` の最小値を `Weak` で与えます。これにより DiagramSymbol 自体が原点に固定され、最小サイズを持つコンテナとして機能します。

ユーザシンボルの追加時には、この DiagramSymbol を包含する `enclose` 制約が自動的に生成されるため、すべてのシンボルが図の領域内に収まり、図のサイズが依存するようになります。
