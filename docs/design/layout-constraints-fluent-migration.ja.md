...
- `docs/draft/new_constraint_builder.md` に従って `ConstraintsBuilder` を導入済みですが、まだ `LayoutSolver.addConstraint`/`.expression` や旧来の型ラッパーを使って制約を追加している箇所が残っています。
- 旧 API を放置すると `LayoutSolver` の公開面が肥大化し、ヒントやテストのような下流コードが古い API と新しい API の両方を気にする必要が出てきます。
- そこで、残る箇所をビルダーへ移し替えたうえで、型・エクスポートの整理も含めた移行手順をドキュメント化して共有します。

## 目的

1. `src/layout` 以下にある制約構築ロジック（`LayoutConstraints`／ガイド／テストなど）をすべて `ConstraintsBuilder` 経由で発行する。
...
- **実施内容**：`arrange*`、`align*`、`enclose*` などのユーティリティが `LayoutSolver.expression`/`addConstraint` を呼ばず、`ConstraintsBuilder` を一貫して利用するように見直します。`expr(...).eq(...).strong()` といったフローを明示し、生成された `rawConstraints` を `record()` に渡す構造を維持します。
...
- **受け入れ条件**：`LayoutSolver.addConstraint`/`.expression` を呼ぶコードが `constraints_builder.ts` 以外に存在しないこと、ガイド／テストから `LayoutContext.getSolver()` を経由して solver を直接操作しないこと。
...
- **実施内容**：
  ...
  3. `src/layout` 内部の `kiwi` インポートを見直し、未使用の型や関数を削除する。
- **受け入れ条件**：必要最低限の型・定数だけが `src/layout` から公開され、旧 API を仮置きするコードが残らない。
...
- `LayoutSolver` の旧ヘルパーを削るのは最終段階まで待ち、すべての制約コードがビルダー経由になっていることを確認してから行う。
