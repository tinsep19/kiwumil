# LayoutSolver の SuggestHandle API

以前は `LayoutSolver#addEditVariable(variable: LayoutVar, strength: kiwi.Strength)` という
...
handle.suggest(10) // internally call kiwi.Solver.suggestValue(x, 10)
handle.strength()  // => "strong" (returns string to isolate kiwi.Strength)
handle.dispose()   // internally call kiwi.Solver.removeEditVariable(x)
```
