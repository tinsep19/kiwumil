 ...
 */
export function createArrangeHorizontalSpec(targets: HintTarget[], gap: number): ConstraintSpec {
  return (builder) => {
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createArrangeVerticalSpec(targets: HintTarget[], gap: number): ConstraintSpec {
  return (builder) => {
    for (let i = 0; i < targets.length - 1; i++) {
      const current = targets[i]!
      const next = targets[i + 1]!
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignLeftSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignRightSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
    if (targets.length === 0) return
    const refBounds = targets[0]!.bounds
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignTopSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
    if (targets.length === 0) return
    const refBounds = targets[0]!.bounds
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignBottomSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
    if (targets.length === 0) return
    const refBounds = targets[0]!.bounds
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignCenterXSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
    if (targets.length === 0) return
    const refBounds = targets[0]!.bounds
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignCenterYSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
    if (targets.length === 0) return
    const refBounds = targets[0]!.bounds
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignWidthSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
    if (targets.length === 0) return
    const refBounds = targets[0]!.bounds
 ...
 * @returns ConstraintSpec function for use with setConstraint
 */
export function createAlignHeightSpec(targets: HintTarget[]): ConstraintSpec {
  return (builder) => {
    if (targets.length === 0) return
    const refBounds = targets[0]!.bounds
